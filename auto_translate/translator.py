...
from auto_translate.google_translate import GoogleTranslator
...
async def translate_document(source_path, target_path, source_language, target_language):
    os.makedirs(path.dirname(source_path), exist_ok=True)
    print(f'Translating {source_path} to {target_path}')
...
async def main():
    ...
    for source_path, target_path in all_files:
        progress.set_description(f'Translating {source_path} to {target_path}')
        translated = await translate_document(source_path, target_path, source_language, target_language)
